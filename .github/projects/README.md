...
| Campo | Tipo | Opções |
|---|---|---|
| **Status da Contribuição** | Select | `Hunting 🔍` · `Draft 📝` · `In Review 👀` · `Merged ✅` · `Rejected ❌` · `Abandoned ⚠️` |
| **Link do PR** | Texto | URL do Pull Request no repositório upstream |
| **Repositório Pai** | Texto | Ex: `nestjs/nest`, `hashicorp/terraform` |
| **Fase** | Select | `PHASE_2026` · `PHASE_2027` · `PHASE_2028` · `PHASE_2029` |
| **Tipo de Contribuição** | Select | `Bug Fix 🐛` · `Feature 🚀` · `Docs 📚` · `Refactor ♻️` · `Test 🧪` · `Chore 🔧` |
...
🗣️ Idioma C1/N2    — Inglês C1 (IELTS 7.0+) e Japonês N2 (JLPT)
